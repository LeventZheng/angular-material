...
export class Ng2Uploader {
  ...
  maxFileSize:number;//单位 M
  selfHandle: boolean = false;
  _queue: any[] = [];
  _emitter: EventEmitter<any> = new EventEmitter();
  _previewEmitter: EventEmitter<any> = new EventEmitter();

  setOptions(options: any): void {
    this.selfHandle = options.selfHandle != null ? options.selfHandle : this.selfHandle;
    this.maxFileSize = options.maxFileSize != null ? options.maxFileSize : this.maxFileSize;
  ...
  uploadFile(file: any): void {
    if(this.selfHandle) return this._emitter.emit(file);
    ...
    if (this.authToken) {
      xhr.setRequestHeader('access-token', `${this.authToken}`);
    }
...
export class UploadRejected {
  ...
  public static get MAX_SIZE_EXCEEDED():string { return '上传的文件不能超过'; }
